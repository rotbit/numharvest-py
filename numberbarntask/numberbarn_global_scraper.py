...
class NumberbarnGlobalExtractor:
    """专门用于从 numberbarn.com 提取 global 号码和价格的爬虫"""
    ...
    async def extract_numbers_from_url(self, page, url: str, country: str) -> List[Dict]:
        """从指定URL提取号码和价格数据，支持翻页"""
        all_numbers = []
        page_number = 1

        try:
            print(f"正在处理 global country: {country}")
            print(f"访问URL: {url}")
            ...
            while page_number <= max_pages:
                ...
                html = await page.content()
                self._save_html_snapshot(page.url, html, meta={"country": country, "page": page_number})
            ...
            print(f"  Country {country} 总共提取到 {len(all_numbers)} 个号码（{page_number} 页）")
            return all_numbers
    ...
    async def extract_from_countries(self, countries: List[str], limit: int = DEFAULT_LIMIT) -> List[Dict]:
        """从给定的 country 列表提取号码数据"""
        ...
        async with async_playwright() as p:
            ...
            try:
                for i, country in enumerate(countries):
                    country = str(country)
                    print(f"\n处理进度: {i+1}/{len(countries)} - Country {country}")

                    url = (
                        "https://www.numberbarn.com/search?type=global"
                        f"&country={country}&moreResults=true&sort=price%2B&limit={limit}"
                    )

                    try:
                        numbers = await self.extract_numbers_from_url(page, url, country)
                    ...
                    if i % 5 == 4:
                        print(f"已处理 {i+1} 个 Country，暂停3秒...")
                        await page.wait_for_timeout(3000)
    ...
    async def extract_single_country(self, country: str, limit: int = DEFAULT_LIMIT) -> List[Dict]:
        """从单个 country 提取号码数据"""
        async with async_playwright() as p:
            browser = await p.chromium.launch(headless=True)
            page = await browser.new_page()
            ...
            try:
                ...
                return await self.extract_numbers_from_url(page, url, country)
            finally:
                await browser.close()

    def run(self, countries: Optional[List[str]] = None, limit: int = DEFAULT_LIMIT) -> List[Dict]:
        """主函数，对外提供 run 接口。"""
        return asyncio.run(self.extract_from_countries(countries or [DEFAULT_COUNTRY], limit=limit))
...
def extract_from_single_country(
    ...
    async def _extract():
        extractor = NumberbarnGlobalExtractor(use_mongodb=use_mongodb, max_pages=max_pages)
        try:
            return await extractor.extract_single_country(country, limit=limit)
        finally:
...
def extract_from_all_global(
    ...
    async def _extract():
        extractor = NumberbarnGlobalExtractor(use_mongodb=use_mongodb, max_pages=max_pages)
        try:
            return await extractor.extract_from_countries(countries or [DEFAULT_COUNTRY], limit=limit)
        finally:
            extractor.close_mongodb()
...
async def main():
    """主函数 - 执行 global 号码提取"""
    extractor = NumberbarnGlobalExtractor()

    try:
        print("开始执行 global 号码提取任务...")
        all_numbers = await extractor.extract_from_countries([DEFAULT_COUNTRY])
